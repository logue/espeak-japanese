...
self.onmessage = function(e) {
	...
	case 'init':
		...
		break;
	case 'encode':
		var buffer = e.data.buf, mp3data;
		if (typeof buffer === 'array'){
			mp3data = Lame.encode_buffer_ieee_float(mp3codec, buffer[0], buffer[1]);
		}else{
			mp3data = Lame.encode_buffer_ieee_float(mp3codec, buffer, buffer);
		}
