...
igo.Tagger.prototype = {
	...
	parseImpl: function (text) {
		var length = text.length,
			nodesAry = new Array(length + 1),
			wdc = this.wdc,
			unk = this.unk,
			tagger = this,
			ml = new igo.MakeLattice(nodesAry, function (vn, prevs) {
	...
	getNBestPath: function (nedesAry, best) {
		var mtx = this.mtx,
			bests = [],
			heap = new jsheap(function (a, b) {
				return a.predict_cost < b.predict_cost;
			}),
			eos = nedesAry[nedesAry.length - 1][0];

		heap.push({
			node: eos,
			cost: 0,
			predict_cost: eos.cost,
			next: undefined
		});
		while (!heap.empty() && bests.length < best) {
			var n = heap.top(),
				leftId = n.node.leftId,
				prevs = n.node.prevs;
...
igo.Unknown.prototype.search = function (text, start, wdic, callback) {
	var category = this.category,
		...
		limit = Math.min(length, ct.length + start);
...
igo.Searcher.prototype = {
	...
	eachCommonPrefix: function (key, start, fn) {
		var base = this.base,
			...
			node = base.get(0),
			offset = -1,
			kin = new igo.KeyStream(key, start);

		while (true) {
			var code = kin.read().charCodeAt(0),
				terminalIdx = node + igo.Node.Chck.TERMINATE_CODE,
				idx = node + code;
	...
	keyExists: function (kin, node) {
		var nodeId = Node.Base.ID(node),
			beg = this.begs.get(nodeId),
			s = this.tail.substring(beg, beg + this.lens.get(nodeId));
		return kin.rest() === s;
	}
};
